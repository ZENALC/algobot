...
from algobot.graph_helpers import (
    add_data_to_plot,
    destroy_graph_plots,
...
from algobot.interface.utils import (
    add_to_table,
    clear_table,
    create_popup,
    open_from_msg_box,
...
class Interface(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.threads: Dict[int, QRunnable or None] = {
            BACKTEST: None,
            SIMULATION: None,
            LIVE: None,
            OPTIMIZER: None,
        ...
        self.graphs = (
            {
                ...
                "plots": [],
                "label": self.simulationCoordinates,
                "enable": True,
            },
            {
                "graph": self.backtestGraph,
                "plots": [],
                "label": self.backtestCoordinates,
                "enable": True,
            },
            {
                "graph": self.liveGraph,
                "plots": [],
                "label": self.liveCoordinates,
                "enable": True,
            },
            {
                "graph": self.avgGraph,
                "plots": [],
                "label": self.liveAvgCoordinates,
                "enable": True,
            },
            {
                "graph": self.simulationAvgGraph,
                "plots": [],
                "label": self.simulationAvgCoordinates,
                "enable": True,
            },
        )
        ...
        if algobot.CURRENT_VERSION != algobot.LATEST_VERSION:
            if algobot.LATEST_VERSION != "unknown":
                self.add_to_live_activity_monitor(
                    f"Update {algobot.LATEST_VERSION} is available."
                )
            else:
                self.add_to_live_activity_monitor(
    ...
    def news_thread(self):
        ...
        newsThread.signals.restore.connect(
            lambda: self.refreshNewsButton.setEnabled(True)
        )
    ...
    def news_thread_error(self, e: str):
        ...
        if "www.todayonchain.com" in e:
            create_popup(
                self, "Failed to retrieve latest news due to a connectivity error."
            )
        else:
            create_popup(self, e)
    ...
    def tickers_thread(self):
        ...
        tickerThread.signals.restore.connect(
            lambda: self.configuration.updateTickers.setEnabled(True)
        )
    ...
    def get_tickers() -> List[str]:
        ...
        tickers = [
            ticker["symbol"]
            for ticker in Data(
                loadData=False, log=False
            ).binanceClient.get_all_tickers()
        ]
    ...
    def setup_tickers(self, tickers: List[str]):
        ...
        tickerWidgets = [
            config.tickerLineEdit,
            config.backtestTickerLineEdit,
            config.simulationTickerLineEdit,
    ...
    def setup_news(self, news: List[str]):
        ...
        self.newsStatusLabel.setText(
            f'Retrieved news successfully. (Updated: {now.strftime("%m/%d/%Y, %H:%M:%S")})'
        )
    ...
    def export_optimizer(self, file_type: str):
        ...
        if self.optimizer:
            if len(self.optimizer.optimizerRows) > 0:
                ...
                defaultFileName = self.optimizer.get_default_result_file_name(
                ...
                if open_from_msg_box(
                    text="Do you want to open the optimization report?",
            ...
            create_popup(
                self, "No table rows found because optimizer has not been run yet."
            )
    ...
    def validate_optimizer_or_backtest(self, caller):
        ...
        if config.optimizer_backtest_dict[caller]["data"] is None:
            create_popup(
                self,
                f"No data setup yet for {noun}. Please download or import data in settings first.",
            )
            return False

        selected_symbol = self.interfaceDictionary[caller]["configuration"][
            "ticker"
        ].text()
        ...
        if selected_symbol != download_symbol and download_symbol.lower() != "imported":
            create_popup(
                self,
                f"{noun.capitalize()} symbol ({selected_symbol}) does not match downloaded symbol "
        ...
        selected_interval = (
            self.interfaceDictionary[caller]["configuration"]["interval"]
            .currentText()
            .lower()
        )
        download_interval = config.optimizer_backtest_dict[caller][
            "dataInterval"
        ].lower()

        if (
            selected_interval != download_interval
            and download_symbol.lower() != "imported"
        ):
            create_popup(
                self,
                f"{noun.capitalize()} interval ({selected_interval}) does not match downloaded interval "
        ...
        if download_symbol.lower() == "imported":
            ...
            create_popup(
                self,
                "You are using imported data. Please ensure they logically work correctly with your "
    ...
    def initiate_optimizer(self):
        ...
        if combos["strategies"] == {}:
            create_popup(
                self,
                "No strategies found. Make sure you have some strategies for optimization.",
            )
            return
        ...
        self.threads[OPTIMIZER] = optimizerThread.OptimizerThread(
            gui=self, logger=self.logger, combos=combos
        )
        worker = self.threads[OPTIMIZER]
        worker.signals.started.connect(
            lambda: self.set_optimizer_buttons(running=True, clear=True)
        )
        worker.signals.restore.connect(
            lambda: self.set_optimizer_buttons(running=False, clear=False)
        )
        ...
        if self.configuration.enabledOptimizerNotification.isChecked():
            worker.signals.finished.connect(
                lambda: self.inform_telegram(
                    "Optimizer has finished running.", stop_bot=True
                )
            )
        worker.signals.activity.connect(
            lambda data: add_to_table(
                self.optimizerTableWidget, data=data, insertDate=False
            )
        )
    ...
    def initiate_backtest(self):
        ...
        self.threads[BACKTEST] = backtestThread.BacktestThread(
            gui=self, logger=self.logger
        )
        ...
        worker.signals.message.connect(
            lambda message: self.add_to_monitor(BACKTEST, message)
        )
        worker.signals.restore.connect(
            lambda: self.disable_interface(disable=False, caller=BACKTEST)
        )
        worker.signals.updateGraphLimits.connect(
            lambda x: update_backtest_graph_limits(gui=self, limit=x)
        )
        self.threadPool.start(worker)
    ...
    def end_backtest(self):
        ...
        defaultFile = os.path.join(
            innerPath, self.backtester.get_default_result_file_name()
        )
        fileName, _ = QFileDialog.getSaveFileName(
            self, "Save Result", defaultFile, "TXT (*.txt)"
        )
        ...
        if not fileName:
            ...
            if open_from_msg_box(
                text=f"Backtest results have been saved to {path}.",
    ...
    def update_backtest_gui(
        self, updatedDict: dict, add_data: bool = True, update_progress_bar: bool = True
    ):
        """
    ...
    def update_backtest_activity_based_on_graph(self, position: int, aux: int = -1):
        ...
        if self.backtester is not None:
            if 1 <= position + aux < len(self.backtester.pastActivity):
                try:
                    self.update_backtest_gui(
                        self.backtester.pastActivity[position + aux],
                        add_data=False,
    ...
    def setup_backtester(self, configurationDictionary: dict):
        ...
        self.add_to_backtest_monitor(
            f"Started backtest with {symbol} data and {interval.lower()} interval periods."
        )
    ...
    def check_strategies(self, caller: int) -> bool:
        ...
        if not get_strategies(self.configuration, caller):
            if caller == BACKTEST:
                ...
                message = (
                    "No strategies found. Did you want to day-trade this simulation?"
                )
            elif caller == LIVE:
                message = (
                    "No strategies found. Did you want to day-trade this live bot?"
                )
            else:
    ...
    def validate_ticker(self, caller: int):
        ...
        """
        selected_ticker = self.interfaceDictionary[caller]["configuration"][
            "ticker"
        ].text()
        ...
        if selected_ticker not in self.tickers:
            create_popup(
                self,
                f'Invalid ticker "{selected_ticker}" provided. If it is valid, '
    ...
    def initiate_bot_thread(self, caller: int):
        ...
        worker.signals.addTrade.connect(
            lambda trade: self.update_trades_table_and_activity_monitor(trade, caller)
        )
        worker.signals.restore.connect(
            lambda: self.disable_interface(disable=False, caller=caller)
        )
        ...
        worker.signals.removeCustomStopLoss.connect(
            lambda: self.set_custom_stop_loss(LIVE, False)
        )
        self.threadPool.start(worker)
    ...
    def download_progress_update(self, value: int, message: str, caller):
        ...
        if caller == SIMULATION:
            self.simulationDownloadProgress.setText(
                f"Completion: {value}% {message.lower()}"
            )
        elif caller == LIVE:
    ...
    def end_bot_thread(self, caller):
        ...
        self.disable_interface(
            True, caller=caller, everything=True
        )  # Disable everything until everything is done.
            ...
            if self.trader:
                ...
                if self.configuration.chatPass:
                    self.telegramBot.send_message(
                        self.configuration.telegramChatID.text(), "Bot has been ended."
                    )
                if self.telegramBot:
                    self.telegramBot.stop()
                    self.telegramBot = None
    ...
    def end_crash_bot_and_create_popup(self, caller: int, msg: str):
        ...
        if caller == LIVE:
            ...
            if self.lowerIntervalData and not self.lowerIntervalData.downloadCompleted:
                self.download_progress_update(
                    value=0,
                    message="Lower interval data download failed.",
                    caller=caller,
                )
        elif caller == SIMULATION:
            self.simulationRunningLive = False
            if (
                self.simulationLowerIntervalData
                and not self.simulationLowerIntervalData.downloadCompleted
            ):
                self.download_progress_update(
                    value=0,
                    message="Lower interval data download failed.",
        ...
        if trader and caller != BACKTEST and not trader.dataView.downloadCompleted:
            self.download_progress_update(
                value=0, message="Download failed.", caller=caller
            )
    ...
    def initial_bot_ui_setup(self, caller):
        ...
        if traderPosition is not None:
            self.add_to_monitor(
                caller,
                f"Detected {trader.get_position_string().lower()} position before bot run.",
            )
        ...
        self.statistics.initialize_tab(
            trader.get_grouped_statistics(), tabType=get_caller_string(caller)
        )
    ...
    def disable_interface(self, disable: bool, caller, everything: bool = False):
        ...
        self.interfaceDictionary[caller]["mainInterface"]["runBotButton"].setEnabled(
            disable
        )

        tab = self.configuration.get_category_tab(caller)
        for strategy_name in self.configuration.strategies.keys():
            self.configuration.strategyDict[tab, strategy_name, "groupBox"].setEnabled(
                disable
            )

        if everything:
            self.interfaceDictionary[caller]["mainInterface"][
                "endBotButton"
            ].setEnabled(disable)
        else:
            self.interfaceDictionary[caller]["mainInterface"][
                "endBotButton"
            ].setEnabled(not disable)
    ...
    def update_interface_text(self, caller: int, valueDict: dict):
        ...
        mainInterfaceDictionary["positionValue"].setText(
            valueDict["currentPositionValue"]
        )
    ...
    def update_main_interface_and_graphs(self, caller: int, valueDict: dict):
        ...
        if (
            self.graphUpdateSchedule[index] is None
            or time.time() > self.graphUpdateSchedule[index]
        ):
    ...
    def exit_position_thread(self, caller, humanControl: bool):
        ...
        if trader.currentPosition == LONG:
            if humanControl:
                ...
                trader.sell_long(
                    "Exited long because of override and resumed autonomous logic.",
                    force=True,
                )
        elif trader.currentPosition == SHORT:
            if humanControl:
                ...
                trader.buy_short(
                    "Exited short because of override and resumed autonomous logic.",
                    force=True,
                )
    ...
    def exit_position(self, caller, humanControl: bool = True):
        ...
        thread = workerThread.Worker(
            lambda: self.exit_position_thread(caller=caller, humanControl=humanControl)
        )
        thread.signals.started.connect(
            lambda: self.enable_override(caller=caller, enabled=False)
        )
        thread.signals.finished.connect(
            lambda: self.set_exit_position_gui(caller=caller, humanControl=humanControl)
        )
        thread.signals.restore.connect(
            lambda: self.enable_override(caller=caller, enabled=True)
        )
        thread.signals.error.connect(lambda x: create_popup(self, x))
        self.threadPool.start(thread)
    ...
    def pause_or_resume_bot(self, caller):
        ...
        """
        trader = self.get_trader(caller)
        pauseButton = self.interfaceDictionary[caller]["mainInterface"][
            "pauseBotButton"
        ]
    ...
    def set_advanced_logging(self, boolean: bool):
        ...
        if self.advancedLogging:
            self.add_to_live_activity_monitor(
                "Logging method has been changed to advanced."
            )
            self.add_to_simulation_activity_monitor(
                "Logging method has been changed to advanced."
            )
        else:
            self.add_to_live_activity_monitor(
                "Logging method has been changed to simple."
            )
            self.add_to_simulation_activity_monitor(
                "Logging method has been changed to simple."
            )

    def set_custom_stop_loss(
        self, caller, enable: bool = True, foreignValue: float or None = None
    ):
        """
        ...
        if enable:
            if foreignValue is None:
                ...
                mainDict["customStopLossValue"].setValue(
                    round(foreignValue, trader.precision)
                )
    ...
    def update_trades_table_and_activity_monitor(self, trade: dict, caller):
        ...
        if (
            caller == LIVE
            and self.telegramBot
            and self.configuration.enableTelegramSendMessage.isChecked()
        ):
    ...
    def closeEvent(self, event):
        ...
        ret = qm.question(
            self,
            "Close?",
            f"{message} Are you sure you want to end Algobot?",
            qm.Yes | qm.No,
        )
    ...
    def update_binance_values(self):
        ...
        if self.trader is not None:
            ...
            thread.signals.finished.connect(
                lambda: create_popup(self, "Successfully updated values.")
            )
            thread.signals.error.connect(lambda x: create_popup(self, x))
            self.threadPool.start(thread)
        else:
    ...
    def open_binance(self, caller: int = None):
        ...
        if caller is None:
            ...
            if (
                symbol.strip() == "" or "USDT" not in symbol
            ):  # TODO: Add more ticker pair separators.
                webbrowser.open("https://www.binance.com/en")
            else:
                index = symbol.index("USDT")
                symbol = (
                    f"USDT_{symbol[4:]}" if index == 0 else f"{symbol[:index]}_USDT"
                )
    ...
    def open_trading_view(self, caller: int = None):
        ...
        if caller is None:
            ...
            if symbol.strip() == "":
                ...
                webbrowser.open(
                    f"https://www.tradingview.com/symbols/{symbol}/?exchange=BINANCE"
                )
    ...
    def export_trades(self, caller):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Trades", defaultFile, "CSV (*.csv)"
        )
    ...
    def import_trades(self, caller):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Trades", path, "CSV (*.csv)"
        )
        ...
        try:
            ...
            label.setText(
                "Could not import trade history due to data corruption or no file being selected."
            )
            self.logger.exception(str(e))
