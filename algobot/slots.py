...
def create_configuration_slots(app, gui):
    ...
    """
    gui.configuration.lightModeRadioButton.toggled.connect(
        lambda: set_light_mode(app, gui)
    )
    gui.configuration.darkModeRadioButton.toggled.connect(
        lambda: set_dark_mode(app, gui)
    )
    gui.configuration.bloombergModeRadioButton.toggled.connect(
        lambda: set_bloomberg_mode(app, gui)
    )
    gui.configuration.bullModeRadioButton.toggled.connect(
        lambda: set_bull_mode(app, gui)
    )
    gui.configuration.bearModeRadioButton.toggled.connect(
        lambda: set_bear_mode(app, gui)
    )
    gui.configuration.simpleLoggingRadioButton.clicked.connect(
        lambda: gui.set_advanced_logging(False)
    )
    gui.configuration.advancedLoggingRadioButton.clicked.connect(
        lambda: gui.set_advanced_logging(True)
    )
...
def create_action_slots(gui):
    ...
    """
    gui.otherCommandsAction.triggered.connect(
        lambda: show_and_bring_window_to_front(gui.otherCommands)
    )
    gui.configurationAction.triggered.connect(
        lambda: show_and_bring_window_to_front(gui.configuration)
    )
    gui.aboutAlgobotAction.triggered.connect(
        lambda: show_and_bring_window_to_front(gui.about)
    )
    ...
    gui.openBacktestResultsFolderAction.triggered.connect(
        lambda: open_folder("Backtest Results")
    )
    gui.openOptimizerResultsFolderAction.triggered.connect(
        lambda: open_folder("Optimizer Results")
    )
    gui.openVolatilityResultsFolderAction.triggered.connect(
        lambda: open_folder("Volatility Results")
    )
    ...
    gui.openCredentialsFolderAction.triggered.connect(
        lambda: open_folder("Credentials")
    )
    gui.openConfigurationsFolderAction.triggered.connect(
        lambda: open_folder("Configuration")
    )
    gui.sourceCodeAction.triggered.connect(
        lambda: webbrowser.open("https://github.com/ZENALC/algobot")
    )
    gui.tradingViewLiveAction.triggered.connect(lambda: gui.open_trading_view(LIVE))
    gui.tradingViewSimulationAction.triggered.connect(
        lambda: gui.open_trading_view(SIMULATION)
    )
    gui.tradingViewBacktestAction.triggered.connect(
        lambda: gui.open_trading_view(BACKTEST)
    )
    ...
    gui.wikiAction.triggered.connect(
        lambda: webbrowser.open("https://github.com/ZENALC/algobot/wiki")
    )
