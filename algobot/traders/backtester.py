...
from algobot.enums import BACKTEST, LONG, OPTIMIZER, SHORT, Trend
...
class Backtester(Trader):
    ...
    def main_logic(self):
        ...
        if self.currentPosition == SHORT:
            ...
            if trend == Trend.BULLISH and (self.previousPosition != LONG or not self.stopLossExit):
                self.buy_long('Entered long because a bullish trend was detected.')
                self.reset_smart_stop_loss()
            elif self.marginEnabled and trend == Trend.BEARISH and self.previousPosition != SHORT:
                self.sell_short('Entered short because a bearish trend was detected.')
                self.reset_smart_stop_loss()
