...
class VolatilitySnooperThread(QRunnable):
    def __init__(self, periods, interval, volatility, tickers, filter_word=None):
        ...
        self.tickers = self.get_filtered_tickers(
            tickers=tickers, filter_word=filter_word
        )
    ...
    def get_starting_timestamp(self, multiplier: int = 1) -> int:
        ...
        period_microseconds = (
            period_minutes * 60 * 1000 * (self.periods + 1)
        )  # Using +1 for safety.
    ...
    def snoop(self):
        ...
        for index, ticker in enumerate(self.tickers):
            ...
            data = self.binanceClient.get_historical_klines(
                ticker, self.short_interval, self.get_starting_timestamp()
            )
            ...
            while len(data) < self.periods + 1:
                ...
                data = self.binanceClient.get_historical_klines(
                    ticker, self.short_interval, starting_timestamp
                )
                multiplier += 1
            ...
            if impossible:
                volatility_dict[
                    ticker
                ] = "Not enough data. Maybe the ticker is too new."
            else:
                data = [get_normalized_data(d) for d in data]
                volatility_dict[ticker] = self.volatility_func(
                    periods=self.periods, data=data
                )
