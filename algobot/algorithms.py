...
def get_moving_average(
    ...
    if moving_average == "WMA":
        return get_wma(
            data=moving_data,
            prices=moving_average_periods,
            parameter=moving_average_parameter,
        )
        ...
        raise ValueError(
            "Invalid moving average provided. Available are: EMA, SMA, and WMA."
        )


def get_wma(
    data: List[Dict[str, float]], prices: int, parameter: str, desc: bool = False
) -> float:
    """
...
def get_sma(data: List[Dict[str, float]], prices: int, parameter: str) -> float:
    ...
    return (
        sum(
            [
                get_data_from_parameter(data=period, parameter=parameter)
                for period in data
            ]
        )
        / prices
...
def get_ema(
    ...
    if memo and prices in memo and parameter in memo[prices]:
        ...
        if desc:
            ...
            data = data[: len(data) - sma_prices][
                ::-1
            ]  # Reverse the data to start from back to front.
        else:
...
def get_money_flow_index(periods: int, data: List[Dict[str, float]]) -> float:
    ...
    for period in data[-periods - 1 :]:
        ...
        if previous_typical_price is not None:
            ...
            ):  # If the typical price equals the previous one, then skip.
...
def get_accumulation_distribution_indicator(
    data: Dict[str, float], option: str = "bollinger"
) -> float:
    """
    ...
    if option.lower() == "bollinger":
        ...
        raise ValueError(
            "Please input either bollinger or investopedia for option type."
        )


def get_normal_volume_oscillator(
    periods: int, ad_cache: List[float], data: List[Dict[str, float]]
) -> Union[float, None]:
    """
...
def get_normalized_intraday_intensity(
    periods: int, intraday_intensity_cache: List[float], data: List[Dict[str, float]]
) -> Union[float, None]:
    """
...
def get_basic_volatility(
    periods: int,
    data: List[Dict[str, float]],
    use_returns: bool = True,
    stdev_type: str = "population",
) -> float:
    """
...
def get_zh_volatility(
    periods: int, data: List[Dict[str, float]], stdev_type: str = "population"
) -> float:
    """
...
def get_bollinger_bands(
    moving_average_periods: int,
    volatility_look_back_periods: int,
    volatility: str,
    bb_coefficient: float,
    moving_average: str,
    moving_average_parameter: str,
    data: List[Dict[str, float]],
    use_returns: bool = True,
    dictionary: dict = None,
    stdev_type: str = "sample",
    ...
    """
    middle_band = get_moving_average(
        moving_average, moving_average_parameter, moving_average_periods, data
    )
    volatility = volatility.lower()
    if volatility == "zh" or "yang zhang" in volatility:
        volatility_measure = get_zh_volatility(
            periods=volatility_look_back_periods, data=data, stdev_type=stdev_type
        )
    elif volatility == "rs" or "rogers satchell" in volatility:
        volatility_measure = get_rs_volatility(
            periods=volatility_look_back_periods, data=data
        )
    elif volatility == "gk" or "garman-klass" in volatility:
        volatility_measure = get_gk_volatility(
            periods=volatility_look_back_periods, data=data
        )
    elif volatility == "parkinson":
        volatility_measure = get_parkinson_volatility(
            periods=volatility_look_back_periods, data=data
        )
    elif volatility == "basic":
        volatility_measure = get_basic_volatility(
            periods=volatility_look_back_periods,
            data=data,
...
def get_percent_b(
    data: List[Dict[str, float]], bollinger_bands: Tuple[float, float, float]
) -> float:
    """
