...
class Configuration(QDialog):
    ...
    def update_graph_speed(self):
        ...
        self.parent.add_to_live_activity_monitor(
            f"Updated graph plot speed to every {graphSpeed} second(s)."
        )
    ...
    def get_caller_based_on_tab(self, tab: QTabWidget) -> int:
        ...
        if tab == self.categoryTabs[2]:
            ...
            raise ValueError(
                "Invalid tab provided. No known called associated with this tab."
            )
    ...
    @staticmethod
    def helper_get_optimizer(
        optimizer_tab, dictionary: dict, key: str, optimizerTypes: tuple, settings: dict
    ):
        """
        ...
        if dictionary[optimizer_tab, "groupBox"].isChecked():
            ...
            if len(settings[key]) > 0:
                for opt in optimizerTypes:
                    start, end, step = (
                        dictionary[opt, "start"].value(),
                        dictionary[opt, "end"].value(),
    ...
    def get_optimizer_settings(self) -> dict:
        ...
        self.helper_get_optimizer(
            tab, self.lossDict, "lossType", self.lossOptimizerTypes, settings
        )
        self.helper_get_optimizer(
            tab,
            self.takeProfitDict,
            "takeProfitType",
            self.takeProfitOptimizerTypes,
            settings,
        )
    ...
    def create_loss_inputs(
        self, tab: QTabWidget, innerLayout: QLayout, isOptimizer: bool = False
    ):
        """
        ...
        if isOptimizer:
            ...
            for optimizerType in self.lossOptimizerTypes:
                self.lossDict[optimizerType, "start"] = start = get_default_widget(
                    QSpinBox, 1, 0
                )
                self.lossDict[optimizerType, "end"] = end = get_default_widget(
                    QSpinBox, 1, 0
                )
                self.lossDict[optimizerType, "step"] = step = get_default_widget(
                    QSpinBox, 1
                )
                add_start_end_step_to_layout(
                    innerLayout, optimizerType, start, end, step
                )
        else:
            self.lossDict[tab, "lossType"] = lossTypeComboBox = QComboBox()
            self.lossDict[tab, "lossPercentage"] = lossPercentage = QDoubleSpinBox()
            self.lossDict[
                tab, "smartStopLossCounter"
            ] = smartStopLossCounter = QSpinBox()
    ...
    def create_take_profit_inputs(
        ...
        if isOptimizer:
            ...
            for optimizerType in self.takeProfitOptimizerTypes:
                self.takeProfitDict[
                    optimizerType, "start"
                ] = start = get_default_widget(QSpinBox, 1, 0)
                self.takeProfitDict[optimizerType, "end"] = end = get_default_widget(
                    QSpinBox, 1, 0
                )
                self.takeProfitDict[optimizerType, "step"] = step = get_default_widget(
                    QSpinBox, 1
                )
                add_start_end_step_to_layout(
                    innerLayout, optimizerType, start, end, step
            ...
            self.takeProfitDict[
                tab, "takeProfitType"
            ] = takeProfitTypeComboBox = QComboBox()
            self.takeProfitDict[
                tab, "takeProfitPercentage"
            ] = takeProfitPercentage = QDoubleSpinBox()

            takeProfitTypeComboBox.addItems(self.takeProfitTypes)
            takeProfitTypeComboBox.currentIndexChanged.connect(
                lambda: self.update_take_profit_settings(tab)
            )
            takeProfitPercentage.setValue(5)
            takeProfitPercentage.valueChanged.connect(
                lambda: self.update_take_profit_settings(tab)
            )
    ...
    def set_loss_settings(self, caller: int, config: dict):
        ...
        """
        if (
            "lossTypeIndex" not in config
        ):  # We don't have this data in config, so just return.
            return
        ...
        self.lossDict[tab, "smartStopLossCounter"].setValue(
            config["smartStopLossCounter"]
        )
    ...
    def set_take_profit_settings(self, caller: int, config: dict):
        ...
        """
        if (
            "takeProfitTypeIndex" not in config
        ):  # We don't have this data in config, so just return.
            return

        tab = self.get_category_tab(caller)
        self.takeProfitDict[tab, "takeProfitType"].setCurrentIndex(
            config["takeProfitTypeIndex"]
        )
        self.takeProfitDict[tab, "takeProfitPercentage"].setValue(
    ...
    def get_loss_settings(self, caller: int) -> dict:
        ...
        if dictionary[tab, "groupBox"].isChecked():
            lossType = (
                TRAILING
                if dictionary[tab, "lossType"].currentText() == "Trailing"
                else STOP
            )
        else:
            lossType = None
    ...
    def load_strategy_from_config(self, caller: int, strategyName: str, config: dict):
        ...
        self.strategyDict[tab, strategyName, "groupBox"].setChecked(
            config[f"{strategyName.lower()}"]
        )

        while valueCount > len(valueWidgets):
            add_strategy_inputs(
                self.strategyDict, parameters, strategyName, groupBoxLayout, tab
            )
        while valueCount < len(valueWidgets):
