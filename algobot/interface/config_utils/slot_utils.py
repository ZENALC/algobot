...
from algobot.interface.config_utils.data_utils import (
    download_data,
    import_data,
    stop_download,
)
from algobot.interface.config_utils.strategy_utils import (
    add_strategy_buttons,
...
from algobot.interface.config_utils.user_config_utils import (
    copy_config_helper,
    copy_settings_to_backtest,
    copy_settings_to_simulation,
        ...
        for tab in config_obj.categoryTabs:
            ...
            if config_obj.get_caller_based_on_tab(tab) == OPTIMIZER:
                groupBox, groupBoxLayout = get_regular_groupbox_and_layout(
                    f"Enable {strategyName} optimization?"
                )
                ...
                for index, parameter in enumerate(parameters, start=1):
                    ...
                    if (
                        ...
                        or type(parameter) == tuple
                        and parameter[1] in [int, float]
                    ):
                        ...
                        config_obj.strategyDict[
                            strategyName, index, "start"
                        ] = start = get_default_widget(widget, 1)
                        config_obj.strategyDict[
                            strategyName, index, "end"
                        ] = end = get_default_widget(widget, 1)
                        config_obj.strategyDict[
                            strategyName, index, "step"
                        ] = step = get_default_widget(widget, step_val)
                        if type(parameter) == tuple:
                            message = parameter[0]
                        else:
                            message = f"{strategyName} {index}"
                        add_start_end_step_to_layout(
                            groupBoxLayout, message, start, end, step
                        )
                ...
                values, labels = create_strategy_inputs(
                    parameters, strategyName, groupBoxLayout
                )
...
def load_precision_combo_boxes(config_obj):
    combo_boxes = [
        config_obj.precisionComboBox,
        config_obj.simulationPrecisionComboBox,
        config_obj.backtestPrecisionComboBox,
        config_obj.optimizerPrecisionComboBox,
    ]
...
def load_interval_combo_boxes(config_obj):
    ...
    config_obj.backtestIntervalComboBox.currentTextChanged.connect(
    ...
    config_obj.optimizerIntervalComboBox.currentTextChanged.connect(
        lambda: reset_strategy_interval_comboBox(
    ...
    config_obj.optimizerStrategyIntervalCombobox.currentTextChanged.connect(
        lambda: reset_strategy_interval_comboBox(
            ...
            divisor=helpers.get_interval_minutes(
...
def load_slots(config_obj):
    ...
    c.saveConfigurationButton.clicked.connect(
        lambda: save_config_helper(
            config_obj=c,
            caller=LIVE,
            result_label=c.configurationResult,
            func=save_live_settings,
        )
    )
    c.loadConfigurationButton.clicked.connect(
        lambda: load_config_helper(
            config_obj=c,
            caller=LIVE,
            result_label=c.configurationResult,
            func=load_live_settings,
        )
    )

    c.simulationCopySettingsButton.clicked.connect(
        lambda: copy_config_helper(
            config_obj=c,
            caller=SIMULATION,
            result_label=c.simulationCopyLabel,
            func=copy_settings_to_simulation,
        )
    )
    c.simulationSaveConfigurationButton.clicked.connect(
        lambda: save_config_helper(
            config_obj=c,
            caller=SIMULATION,
            result_label=c.simulationConfigurationResult,
            func=save_simulation_settings,
        )
    )
    c.simulationLoadConfigurationButton.clicked.connect(
        lambda: load_config_helper(
            config_obj=c,
            caller=SIMULATION,
            result_label=c.simulationConfigurationResult,
            func=load_simulation_settings,
        )
    )
    ...
    c.backtestCopySettingsButton.clicked.connect(
        lambda: copy_config_helper(
            config_obj=c,
            caller=BACKTEST,
            result_label=c.backtestCopyLabel,
            func=copy_settings_to_backtest,
        )
    )
    c.backtestSaveConfigurationButton.clicked.connect(
        lambda: save_config_helper(
            config_obj=c,
            caller=BACKTEST,
            result_label=c.backtestConfigurationResult,
            func=save_backtest_settings,
        )
    )
    c.backtestLoadConfigurationButton.clicked.connect(
        lambda: load_config_helper(
            config_obj=c,
            caller=BACKTEST,
            result_label=c.backtestConfigurationResult,
            func=load_backtest_settings,
        )
    )
    ...
    c.optimizerSaveConfigurationButton.clicked.connect(
        lambda: save_config_helper(
            config_obj=c,
            caller=OPTIMIZER,
            result_label=c.optimizerConfigurationResult,
            func=save_optimizer_settings,
        )
    )
    c.optimizerLoadConfigurationButton.clicked.connect(
        lambda: load_config_helper(
            config_obj=c,
            caller=OPTIMIZER,
            result_label=c.optimizerConfigurationResult,
            func=load_optimizer_settings,
        )
    )

    c.setBalanceColorButton.clicked.connect(
        lambda: get_and_set_line_color(c, c.balanceColor)
    )
    c.setHoverLineColorButton.clicked.connect(
        lambda: get_and_set_line_color(c, c.hoverLineColor)
    )
    ...
    c.loadCredentialsButton.clicked.connect(
        lambda: load_credentials(config_obj=c, auto=False)
    )
