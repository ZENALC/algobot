...
def helper_get_save_file_path(config_obj, name: str) -> Union[str]:
    ...
    filePath, _ = QFileDialog.getSaveFileName(
        config_obj, f"Save {name} Configuration", defaultPath, "JSON (*.json)"
    )
    return filePath
...
def load_config_helper(config_obj, caller, result_label: QLabel, func: Callable):
    ...
    filePath, _ = QFileDialog.getOpenFileName(
        config_obj, f"Load {caller_str} config", targetPath, "JSON (*.json)"
    )
...
def copy_config_helper(config_obj, caller, result_label, func: Callable):
    ...
    result_label.setText(
        f"Copied all viable settings from main to {get_caller_string(caller)} settings successfully."
    )
...
def copy_settings_to_simulation(config_obj):
    ...
    config_obj.simulationIntervalComboBox.setCurrentIndex(
        config_obj.intervalComboBox.currentIndex()
    )
    config_obj.simulationTickerLineEdit.setText(config_obj.tickerLineEdit.text())
    config_obj.simulationPrecisionComboBox.setCurrentIndex(
        config_obj.precisionComboBox.currentIndex()
    )
...
def copy_settings_to_backtest(config_obj):
    ...
    config_obj.backtestPrecisionComboBox.setCurrentIndex(
        config_obj.precisionComboBox.currentIndex()
    )


def copy_strategy_settings(
    config_obj, fromCaller: int, toCaller: int, strategyName: str
):
    """
    ...
    fromCallerGroupBox = config_obj.strategyDict[
        fromCallerTab, strategyName, "groupBox"
    ]
    config_obj.strategyDict[toCallerTab, strategyName, "groupBox"].setChecked(
        fromCallerGroupBox.isChecked()
    )
    set_strategy_values(
        config_obj,
        strategyName,
        toCaller,
        get_strategy_values(config_obj, strategyName, fromCaller),
    )
...
def copy_loss_settings(config_obj, fromCaller: int, toCaller: int):
    ...
    config_obj.lossDict[toTab, "lossType"].setCurrentIndex(
        config_obj.lossDict[fromTab, "lossType"].currentIndex()
    )
    config_obj.lossDict[toTab, "lossPercentage"].setValue(
        config_obj.lossDict[fromTab, "lossPercentage"].value()
    )
    config_obj.lossDict[toTab, "smartStopLossCounter"].setValue(
        config_obj.lossDict[fromTab, "smartStopLossCounter"].value()
    )

    if toTab != config_obj.backtestConfigurationTabWidget:
        config_obj.lossDict[toTab, "safetyTimer"].setValue(
            config_obj.lossDict[fromTab, "safetyTimer"].value()
        )
