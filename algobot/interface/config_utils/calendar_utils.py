...
def get_calendar_dates(
    config_obj, caller: int = BACKTEST
) -> Tuple[datetime.date or None, datetime.date or None]:
    """
    ...
    """
    startDate = (
        config_obj.optimizer_backtest_dict[caller]["startDate"]
        .selectedDate()
        .toPyDate()
    )
    endDate = (
        config_obj.optimizer_backtest_dict[caller]["endDate"].selectedDate().toPyDate()
    )
...
def setup_calendar(config_obj, caller: int = BACKTEST):
    ...
    config_obj.optimizer_backtest_dict[caller]["startDate"].setDateRange(
        qStartDate, qEndDate
    )
    ...
    config_obj.optimizer_backtest_dict[caller]["endDate"].setDateRange(
        qStartDate, qEndDate
    )
