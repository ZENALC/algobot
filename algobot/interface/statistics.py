...
class Statistics(QDialog):
    ...
    def initialize_tab(self, valueDictionary: Dict[str, Any], tabType: str):
        ...
        for categoryKey in valueDictionary:
            self.add_category_and_children_keys(
                categoryKey, valueDictionary, innerTabs, tab
            )
    ...
    @staticmethod
    def add_category_and_children_keys(
        categoryKey: str,
        valueDictionary: Dict[str, Any],
        innerTabs: Dict[str, Any],
        tab: QTabWidget,
    ):
        """
    ...
    @staticmethod
    def set_profit_or_loss_label(
        valueDictionary: Dict[str, Any], innerTabs: Dict[str, Any]
    ):
        """
        ...
        if "general" in valueDictionary and "profit" in valueDictionary["general"]:
            ...
            if "profit" in tab:
                if valueDictionary["general"]["profit"][1] == "-":
                    label = "Loss"
                    valueDictionary["general"]["profit"] = (
                        "$" + valueDictionary["general"]["profit"][2:]
                    )
                else:
                    label = "Profit"
    ...
    def modify_tab(self, valueDictionary: Dict[str, Any], tabType: str):
        ...
        self.set_profit_or_loss_label(
            valueDictionary=valueDictionary, innerTabs=innerTabs
        )
        ...
        for (
            ...
            innerTabs
        ):  # If there's a change in the value dictionary, re-initialize the tab.
