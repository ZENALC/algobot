...
@pytest.mark.parametrize("interval, expected", [("1m", "1 Minute"), ("8h", "8 Hours")])
def test_convert_small_interval(interval: str, expected: str):
    assert (
        convert_small_interval(interval) == expected
    ), f"Expected converted interval to be: {expected}."


@pytest.mark.parametrize("interval, expected", [("1 Minute", "1m"), ("8 Hours", "8h")])
def test_convert_long_interval(interval: str, expected: str):
    assert (
        convert_long_interval(interval) == expected
    ), f"Expected converted interval to be: {expected}."
...
def test_get_label_string(label: str, expected: str):
    assert (
        get_label_string(label) == expected
    ), f"Expected label string to be: {expected}."
...
def test_get_caller_string(caller: int, expected: str):
    assert (
        get_caller_string(caller) == expected
    ), f"Expected caller string to be: {expected}."
...
def test_get_data_from_parameter(data, parameter, expected):
    assert (
        get_data_from_parameter(data, parameter) == expected
    ), f"Expected data to be: {expected}."
...
def test_get_normalized_data(data, date_in_utc, expected):
    assert (
        get_normalized_data(data, date_in_utc) == expected
    ), f"Expected normalized data to be: {expected}."
...
def test_get_ups_and_downs(data, parameter, expected):
    assert (
        get_ups_and_downs(data, parameter) == expected
    ), f"Expected ups and downs to be: {expected}."
...
def test_get_elapsed_time(elapsed, expected):
    assert (
        get_elapsed_time(elapsed) in expected
    ), f"Expected elapsed time to be in: {expected}."
...
def test_parse_strategy_name(name, expected):
    assert (
        parse_strategy_name(name) == expected
    ), f"Expected parsed strategy to be: {expected}."
