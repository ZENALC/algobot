...
class TestBaseTrader(unittest.TestCase):
    ...
    def test_buy_long(self):
        ...
        transaction_fee = (
            self.trader.startingBalance * self.trader.transactionFeePercentageDecimal
        )
        coin = (
            self.trader.startingBalance - transaction_fee
        ) / self.trader.currentPrice
    ...
    def test_sell_long(self):
        ...
        transaction_fee = (
            coin
            * self.trader.currentPrice
            * self.trader.transactionFeePercentageDecimal
        )
        ...
        self.assertEqual(
            self.trader.commissionsPaid, transaction_fee + previous_transaction_fee
        )
    ...
    def test_sell_short(self):
        ...
        self.assertEqual(self.trader.stopLossExit, False)
        self.assertEqual(
            self.trader.balance, self.trader.startingBalance * 2 - transaction_fee
        )
    ...
    def test_buy_short(self):
        ...
        balance = (
            self.trader.balance - coin_owed * self.trader.currentPrice - transaction_fee
        )
        ...
        self.assertEqual(self.trader.balance, balance)
        self.assertEqual(
            self.trader.commissionsPaid, previous_transaction_fee + transaction_fee
        )
        self.assertEqual(self.trader.coin, 0)
    ...
    def test_apply_take_profit_settings(self):
        take_profit_settings = {"takeProfitPercentage": 25, "takeProfitType": STOP}
    ...
    def test_get_stop_loss(self):
        ...
        self.assertEqual(
            self.trader.get_stop_loss(), 10 * (1 - self.trader.lossPercentageDecimal)
        )
    ...
    def test_get_trailing_or_stop_loss_string(self):
        ...
        self.assertEqual(
            self.trader.get_trailing_or_stop_type_string(TRAILING), "Trailing"
        )
    ...
    def test_get_safe_rounded_string(self):
        ...
        self.assertEqual(
            self.trader.get_safe_rounded_string(value=5.12345, roundDigits=5),
            "$5.12345",
        )
        self.assertEqual(
            self.trader.get_safe_rounded_string(value=5.12345, roundDigits=0), "$5.0"
        )
        self.assertEqual(
            self.trader.get_safe_rounded_string(
                value=1.23, roundDigits=2, symbol="*", direction="right", multiplier=5
            ),
            "6.15*",
        )
    ...
    def test_get_take_profit(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid type of take profit type provided."
        ):
    ...
    def test_get_net(self):
        ...
        self.assertEqual(
            self.trader.get_net(), self.trader.currentPrice * self.trader.coin
        )
        ...
        self.assertEqual(
            self.trader.get_net(),
            self.trader.startingBalance - self.trader.commissionsPaid,
        )
